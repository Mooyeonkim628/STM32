...
{
    if (argc != 2) {
        printf("Specify only one file name to play.");
        return;
    }
    ...
    // your command should enable the DAC, V
    //a timer set to the right rate, V
    //and use circular DMA to copy samples from a buffer into the DAC (just like homework 9).
    //Fill the entire buffer with the start of the file and start the timer/DMA.
    //Enable both the half-transfer and the transfer-complete interrupts.
    //In the interrupt handler, read from the file to refill
    //the first half of the buffer with the next part of the file on a half-transfer interrupt,
    //and refill the second half of the buffer on a transfer-complete interrupt.
}
//
struct {
    const char *cmd;
